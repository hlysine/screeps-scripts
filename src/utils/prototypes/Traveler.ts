/**
 * Credit: https://github.com/bonzaiferroni/Traveler
 */
...
interface TravelToOptions {
  ...
  /**
   * SourceKeeper rooms will be included in path. (if false, SK rooms will still be taken if they are they only viable path). Default is false.
   */
  allowSK?: boolean;
  /**
   ...
   * Used for overriding the default PathFinder callback. If it returns false, that room will be excluded. If it returns a matrix,
   * it will be used in place of the default matrix. If it returns undefined the default matrix will be used instead.
   */
   ...
   * Used for overriding the findRoute callback. If it returns a number that value will be used to influence the route.
   * If it returns undefined it will use the default value.
   */
  ...
  /**
   * Can be used to force or prohibit the use of findRoute. If undefined it will use findRoute only for paths that span a
   * larger number of rooms (linear distance >2).
   */
  ...
  maxOps?: number;
  /**
   * Allows you to avoid making a new pathfinding call when your destination is only 1 position away from what it
   * was previously. The new direction is just added to the path. Default is false.
   */
  movingTarget?: boolean;
  /**
   * Will guarantee that a new structure matrix is generated. This might be necessary if structures are likely
   * to change often. Default is false.
   */
  freshMatrix?: boolean;
  /**
  ...
  /**
   * This can improve the chance of finding a path in certain edge cases where might otherwise fail. Default is false.
   */
  ensurePath?: boolean;
}
...
declare global {
  interface Creep {
    /**
     * Move the creep.
     * @param destination A RoomPosition or an object with a property `pos: RoomPosition`.
     * @param ops An optional object
     */
...
export class Traveler {
  ...
  public static travelTo(creep: Creep, destination: HasPos | RoomPosition, options: TravelToOptions = {}): number {
    ...
    if (this.isStuck(creep, state)) {
      state.stuckCount++;
      this.circle(creep.pos, "magenta", state.stuckCount * 0.2);
    } else {
      state.stuckCount = 0;
    }
      ...
      if (state.cpu > REPORT_CPU_THRESHOLD) {
        ...
        console.log(
          `TRAVELER: heavy cpu use: ${creep.name}, cpu: ${
            state.cpu
          } origin: ${creep.pos.toString()}, dest: ${destination.toString()}`
        );
      }
    ...
    if (ret.incomplete && options.ensurePath) {
      if (options.useFindRoute === undefined) {
        ...
        if (roomDistance <= 2) {
          ...
          console.log(
            `from: ${origin.toString()}, destination: ${destination.toString()}, searched {${roomsSearched}} rooms`
          );
